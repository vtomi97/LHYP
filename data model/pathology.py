    ...
    def create_patient(self):
        for pf in [pjoin(self.inputpath, folder_name) for folder_name in os.listdir(self.inputpath)]:
            ...
            if os.path.isdir(la_folder) and len(os.listdir(la_folder)) != 0:
                ...
                result = "NA"
                try:
                    f = open(pjoin(self.inputpath, _id, "meta.txt"), "r")
                    result = f.read().split()[1]
                except OSError:
                    print("Loading of metadata failed! ", _id)
                la2 = 0
                la4 = 0
                lal = 0
                time = 0
                        ...
                        try:
                            ...
                            if la_type == "2CH":
                                if la2 % 3 != 0:
                                    ...
                                    continue
                                la2 += 1
                                patient2.append(self.dicom_reader(temp))
                            elif la_type == "4CH":
                                if la4 % 3 != 0:
                                    la4 += 1
                                    continue
                                la4 += 1
                                patient4.append(self.dicom_reader(temp))
                            elif la_type == "LVOT":
                                if lal % 3 != 0:
                                    lal += 1
                                    continue
                                lal += 1
                                patientl.append(self.dicom_reader(temp))
                        except Exception as e:
                            print("Error loading: ", _id, e)
                if len(patient2) != 0:
                    self.patients2CH.append(Patient(_id, gender, weight, spacing, result, patient2))
                if len(patient4) != 0:
                    self.patients4CH.append(Patient(_id, gender, weight, spacing, result, patient4))
                if len(patientl) != 0:
                    self.patientsLVOT.append(Patient(_id, gender, weight, spacing, result, patientl))
